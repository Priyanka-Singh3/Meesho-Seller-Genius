...
import { fetchProductList, fetchRegionData, fetchFestivalData, applyPriceChanges } from '../../services/dynamicPricingService';
...
const DynamicPricing = () => {
  ...
  const handleProductSelect = (product) => {
    ...
    const adjustments = regionData.map(region => ({
      region: region.name,
      currentPrice: product.price,
      suggestedPrice: calculateSuggestedPrice(product.price, region.demandFactor),
      demand: region.demandFactor > 1 ? 'High' : region.demandFactor < 1 ? 'Low' : 'Normal'
    }));
    setPriceAdjustments(adjustments);
  };

  const calculateSuggestedPrice = (basePrice, demandFactor) => {
    const adjustment = demandFactor > 1 ?
      basePrice * (1 + (demandFactor - 1) * 0.15) :
      demandFactor < 1 ?
      basePrice * (1 - (1 - demandFactor) * 0.1) :
      basePrice;
  ...
  const handleFestivalSelect = (festival) => {
    ...
    if (selectedProduct) {
      const festivalAdjustments = regionData.map(region => ({
        region: region.name,
        currentPrice: selectedProduct.price,
        suggestedPrice: calculateFestivalPrice(selectedProduct.price, festival.impactFactor, region.demandFactor),
        demand: 'High (Festival)'
      }));
      setPriceAdjustments(festivalAdjustments);
    }
  };

  const calculateFestivalPrice = (basePrice, festivalImpact, regionDemand) => {
    const adjustment = basePrice * (1 + festivalImpact * 0.25) * (1 + (regionDemand - 1) * 0.2);
    return Math.round(adjustment * 100) / 100;
  };
      ...
      <div className="flex-1 bg-gradient-to-br from-[#f8f7fa] to-[#ffe4fa]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            <div>
              <h1 className="text-3xl font-extrabold text-[#9B177E]">Dynamic Pricing</h1>
              <p className="mt-2 text-lg text-gray-600">
                Optimize your prices based on regional demand and upcoming festivals
              </p>
            </div>
            <div className="mt-4 md:mt-0">
              <button
                ...
                className={`px-6 py-3 rounded-lg font-semibold text-white ${loading || !selectedProduct ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#9B177E] hover:bg-[#7a125f]'}`}
              >
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-3">
                  <button
                    className="w-full px-4 py-2 bg-[#f3e8ff] hover:bg-[#e5d5f7] text-[#9B177E] rounded-lg text-left"
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-xl shadow-md p-6">
                <div className="flex border-b border-gray-200 mb-6">
                  <button
                    className={`px-4 py-2 font-medium ${activeTab === 'region' ? 'text-[#9B177E] border-b-2 border-[#9B177E]' : 'text-gray-500'}`}
                    onClick={() => setActiveTab('region')}
                  >
                    Regional Demand
                  </button>
                  <button
                    className={`px-4 py-2 font-medium ${activeTab === 'festival' ? 'text-[#9B177E] border-b-2 border-[#9B177E]' : 'text-gray-500'}`}
                    onClick={() => setActiveTab('festival')}
                  >
                    Festival Impact
                  </button>
                  <button
                    className={`px-4 py-2 font-medium ${activeTab === 'history' ? 'text-[#9B177E] border-b-2 border-[#9B177E]' : 'text-gray-500'}`}
                    onClick={() => setActiveTab('history')}
                  >
                    Price History
                  </button>
                  ...
                  <div>
                    <div className="flex space-x-2 mb-4">
                      <button
                        ...
                        className={mapMode === 'demand' ? 'bg-[#9B177E] text-white px-4 py-2 rounded' : 'bg-gray-200 px-4 py-2 rounded'}
                      >
                        Show Demand
                      </button>
                      <button
                        onClick={() => setMapMode('population')}
                        className={mapMode === 'population' ? 'bg-[#9B177E] text-white px-4 py-2 rounded' : 'bg-gray-200 px-4 py-2 rounded'}
                      >
                        Show Population
                      </button>
                    </div>
                      ...
                      <div className="mt-4 p-4 bg-[#f9f5ff] rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-2">
                          <div>
                            ...
                            <span className="block font-medium">{selectedRegion.demandFactor > 1 ? 'High' : 'Normal'}</span>
                          </div>
                          <div>
                            <span className="text-sm text-gray-600">Population:</span>
                            <span className="block font-medium">{selectedRegion.population ? selectedRegion.population : '--'}</span>
                          </div>
                          <div>
                            <span className="text-sm text-gray-600">Suggested Price:</span>
                            <span className="block font-medium">
                              ₹{selectedProduct ? calculateSuggestedPrice(selectedProduct.price, selectedRegion.demandFactor) : '--'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 p-4 bg-gradient-to-r from-[#f3e8ff] to-[#ffe4fa] rounded-lg border-l-4 border-[#9B177E]">
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-2">
                          ...
                          <div>
                            ...
                            <span className="block font-medium">
                              {selectedFestival.impactFactor > 1 ? 'High' : 'Normal'}
                            </span>
                          </div>
                        </div>
                        {selectedProduct && (
                          <div className="mt-3">
                            <span className="text-sm text-gray-600">Suggested Price:</span>
                            <span className="block font-medium">
                              ₹{calculateFestivalPrice(selectedProduct.price, selectedFestival.impactFactor, 1)}
                            </span>
                          </div>
                        )}
                      </div>
